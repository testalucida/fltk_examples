...
public:
	DraggableGroup(int X, int Y, int W, int H, const char *L = 0) :
			Fl_Group(X, Y, W, H, L), xoff(0), yoff(0), drag_index(0), drag_widget(
					0) {
		box(FL_UP_BOX);
	}
...
const char *tt_drag =
		"Drag this DraggableGroup and/or its child groups and squares. "
				"Use the right mouse button (MB3) to drag objects.";
const char *tt_group =
		"You can drag this Fl_Group, but not its children (squares).";
...
void cb(int i) {
	fprintf(stderr, "cb called with argument %d\n", i);
}
...
int main() {
	...
	case 100: {
		Fl_Double_Window win(500, 500,
				"Drag children within their parent group");

		DraggableGroup area(0, 0, 500, 400,
				"Use the right mouse button (MB3)\nto drag objects");
		...
		Fl_Button button2(200, 410, 180, 40,
				"Fl_Button outside DraggableGroup");
